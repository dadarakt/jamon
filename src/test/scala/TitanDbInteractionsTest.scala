...
class TitanDbInteractionsTest extends FunSuite with BeforeAndAfterAll{
  ...
  val sig = JuliaSignature(funcName,
    List( JuliaArgument("a", "Int64", None),
          JuliaArgument("b", "Int32", None),
          JuliaArgument("c", "Double", Some("12")),
          JuliaArgument("d", "Decimal", "\"wurst\"")
    ),
    Some(JuliaArgument("e", "Int32", None))
  )
  ...
  test("Insert something into the graph and make sure it is in the graph") {
    val graph = graphObject.graph
    val vertexOption = TitanDatabaseConnection.insertSourceCode(code, sig, auth, doc, false, false)
    graph.commit()
    println("achtung: " + vertexOption)
    func = graph.query.has(FunctionName, funcName).vertices.head
    meth = func.query.direction(Direction.OUT).vertices.
      find(_.getProperty[String](SignatureString) == sig.signatureString).get
  ...
  test("Override old data in the version and make sure there is no new version in the graph") {
    val graph = graphObject.graph
    TitanDatabaseConnection.insertSourceCode("overwritten", sig, auth, doc, false, false)
    assert(vers.getProperty[String](Code) === "overwritten")
  }
//
//  ignore("Insert a new version, make sure there is a new one and the old one still extists.") {
//    val graph = graphObject.graph
...
//  }
//
//  ignore("Insert a new implementation, make sure there is a new one and the old one still exists.") {
//    val graph = graphObject.graph
...
//  }
//
//  ignore("Do a quick batch insert to get an idea of the time needed") {
//    val numNodes = 1000
...
//  }
//
  ignore("Remove the function introduced to clean up the graph") {
